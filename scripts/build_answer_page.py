...
faq = [
 ...
 ("What are FHA applications denied for?", "Across the 100 largest FHA lenders, the median share of cited denial reasons was: " + "; ".join(f"{n} {v:.1f}%" for n, v in reasons) + ". Reason fields are not a partition, so shares need not sum to 100."),
...
{sec("Why applications are denied", f"<p>Median share of cited denial reasons across the 100 largest FHA lenders, 2025. Reason fields are not a partition, so shares need not sum to 100.</p><table><tr><th>Cited reason</th><th>Median share</th></tr>{reasons_rows}</table>")}
