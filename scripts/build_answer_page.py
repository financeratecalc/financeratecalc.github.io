...
rs = json.load(open("api/denial-reasons-top100.json")); pm = {k: v["median_share_pct"] for k, v in rs["by_reason"].items()}
...
faq = [
 ...
 ("What are FHA applications denied for?", "Across the 95 of the 100 largest FHA lenders that report denial reasons (universe U-TOP100-WITH-REASONS-2025), the median share of cited denial reasons was: " + "; ".join(f"{n} {v:.1f}%" for n, v in reasons) + ". Reason fields are not a partition, so shares need not sum to 100."),
...
{sec("Why applications are denied", f"<p>Median share of cited denial reasons across the 95 of the 100 largest FHA lenders that report reasons, 2025 (universe U-TOP100-WITH-REASONS-2025, <a href='/universes.json'>universes.json</a>). Reason fields are not a partition, so shares need not sum to 100.</p><table><tr><th>Cited reason</th><th>Median share</th></tr>{reasons_rows}</table>")}
