...
An earlier pre-comparison had read the intervention as "doubling" full marks; when the comparable baseline was run, the effect on full marks was zero. That is the reason the baseline run was necessary, and it is kept here as method.

What moved is the mix of failure codes. Codes are not a partition: an answer can carry several (48 codes over 36 answers in both conditions; 12 answers clean in both). Per code, answer-presence counts: FABRICATED_SUPPORT 12 → 8, ATTRIBUTION_DRIFT 10 → 6, SCOPE_MISSING 6 → 4, OVERREACH_FROM_SOURCE 9 → 16. **None of these differences is significant at n = 36** (Fisher exact, two-sided: p = 0.43, 0.40, 0.74, 0.14; any-code 24 vs 24, p = 1.0). The direction — less invented, more copied past the contracted edge — is consistent with the section's first claim, and it is stated as a direction, not a result. Five answers in the control and three with the sentence carried both FABRICATED_SUPPORT and OVERREACH_FROM_SOURCE: the two behaviours coexist in one answer; this is a shift in mix, not a swap.

At the question level the metro gap went P/I/P → C/C/C. Two questions moved the other way: q3 ("which major FHA lender had the lowest denial rate", a name-and-figure lookup with a tie; P/P/P → P/P/I) and q8 ("what share of FHA denials cite incomplete application", a single reason-share lookup; C/C/C → C/P/P). Both wanted one number; the sentence supplied context they did not ask for, and the plausible mechanism is that a contracted sentence invites elaboration exactly where the question wanted none. Three repeats cannot separate that from noise.
